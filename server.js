...
const OneToOneMessage = require("./MODELS/OneToOneMessaging");
...
io.on("connection", async (socket) => {
  ...
  socket.on("friend_request", async (data) => {
    try {
      // Check if there's an existing friend request between the users
      const existingRequest = await FriendRequest.findOne({
        sender: data.from,
        recipient: data.to,
        status: { $ne: 'accepted' }, // Consider requests that are not accepted yet
      });
      const from = await User.findById(data.from).select("socket_id");

      if (existingRequest) {
        io.to(from.socket_id).emit("request_exist", {
          message: "Friend Request is already Sent",
        });
        // You can emit an event or take any action to notify the users
        return "Friend Request is already Sent"; // Stop further execution
      }
      ...
      // Emit events to notify the users
      io.to(from.socket_id).emit("request_accepted", {
        message: "Friend Request Sent",
      });
  
      io.to(to.socket_id).emit("request_accepted", {
  ...
  socket.on("Pin_conversation", async (data) => {
    ...
    const existing_conversations = await OneToOneMessage.find({
      ...
      socket.emit("pin_chat", existing_conversations[0]);
  ...
  socket.on("unPin_conversation", async (data) => {
    ...
    const existing_conversations = await OneToOneMessage.find({
      ...
      socket.emit("unpin_chat", existing_conversations[0]);
  ...
  socket.on("text_message", async (data) => {
    ...
    let fromUser= to_user.firstName+" "+ from_user.lastName
